...
def get_letter_labels(names):
    return _get_labels(names, "lab", dataset_dir)

def get_word_labels(names):
    return _get_labels(names, "lab2", dataset_dir)

def _get_labels(names, ext, dataset_dir):
    return [np.loadtxt(dataset_dir + "LABEL/" + name + "." + ext) for name in names]
