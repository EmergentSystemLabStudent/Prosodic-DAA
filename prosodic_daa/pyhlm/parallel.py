...
def _get_sampled_stateseq_norep_and_durations_censored_prosody(idx):
    grp = args[idx]

    if len(grp) == 0:
        return []

    datas, prosodys, kwargss = zip(*grp)

    states_list = []
    for data, prosody, kwargs in zip(datas, prosodys, kwargss):
        model.add_data(data, prosody, initialize_from_prior=False, **kwargs)
        states_list.append(model.states_list.pop())
