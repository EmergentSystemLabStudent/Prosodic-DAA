...
class WeakLimitProsodicHDPHLMStatesPython(WeakLimitHDPHLMStatesPython):
    ...
    def __init__(self, model, data=None, prosody=None, trunc=None, generate=True, initialize_from_prior=False):
        self.prosody = prosody
        return super(WeakLimitProsodicHDPHLMStatesPython, self).__init__(model, data, trunc, generate, initialize_from_prior)

    @property
    def apBl(self):
        if self._apBl is None:
            apBl = np.empty((self.prosody.shape[0], 2)) #self.model._letter_num_states
            for idx, obs_distn in enumerate(self.model.prosody_distns):
                apBl[:,idx] = obs_distn.log_likelihood(self.prosody).ravel()
    ...
    def messages_backwards(self):
        ...
        for t in range(T-1, -1, -1):
            betastarl[t] = np.logaddexp.reduce(betal[t:t+trunc] + self.cumulative_likelihoods(t, t+trunc) + self.cumulative_likelihoods_prosody(t, t+trunc) + aDl[:min(trunc, T-t)], axis=0)
            betal[t-1] = np.logaddexp.reduce(betastarl[t] + log_trans_matrix, axis=1)
        betal[-1] = 0.0
        normalizerl = np.logaddexp.reduce(betastarl[0] + np.log(pi_0))
        return betal, betastarl, normalizerl

    def cumulative_likelihoods_prosody(self, start, stop):
        T = min(self.T, stop)
        tsize = T - start
        cum_like = np.empty((tsize, self.model.num_states), dtype=np.float64)

        for state, word in enumerate(self.model.word_list):
            cum_like[:, state] = self.likelihood_block_word_prosody(start, stop, word, state)
        return cum_like

    def likelihood_block_word_prosody(self, start, stop, word, state):
        T = min(self.T, stop)
        tsize = T - start
        ...
        L = len(word)
        alphal = np.ones((tsize, L), dtype=np.float64) * -np.inf
        return _log_likelihood_block_word_prosody(apBl, aDl, state, alphal)[:, -1]
    ...
    def sample_forwards(self, betal, betastarl):
        ...
        while t < T:
            ...
            durprob = np.random.random()
            # dur = len(self.model.word_list[state])
            cache_mess_term = np.exp(self.likelihood_block_word(t, T, self.model.word_list[state]) + self.likelihood_block_word_prosody(t, T, self.model.word_list[state], state) + betal[t:T, state] - betastarl[t, state])
            dur = 0
            while durprob > 0 and t+dur < T:
                # p_d_prior = aD[dur, state] if t + dur < T else 1.
                p_d_prior = aD[dur, state]
...
class WeakLimitProsodicHDPHLMStates(WeakLimitProsodicHDPHLMStatesPython):

    def messages_backwards(self):
        return super(WeakLimitProsodicHDPHLMStates, self).messages_backwards()
    ...
    def likelihood_block_word_prosody(self, start, stop, word, state):
        from pyhlm.internals.hlm_messages_interface import internal_messages_forwards_log_prosody
        T = min(self.T, stop)
        tsize = T - start
        apBl = self.apBl[start:T]
        aDl = self.aDl[:tsize]
        ...
        return internal_messages_forwards_log_prosody(apBl, aDl, state, alphal)[:, -1]

    def likelihood_block_word_python(self, start, stop, word):
        return super(WeakLimitProsodicHDPHLMStates, self).likelihood_block_word(start, stop, word)

    def likelihood_block_word_prosody_python(self, start, stop, word, state):
        return super(WeakLimitProsodicHDPHLMStates, self).likelihood_block_word_prosody(start, stop, word, state)

def _log_likelihood_block_word_prosody(apBl, aDl, state, alphal):
    alphal[:] = -np.inf
    if alphal.shape[0] - alphal.shape[1] + 1 <= 0:
        return alphal
    apBl_0 = np.cumsum(np.concatenate(([0], apBl[:,0])))
    alphal[:, -1] = apBl_0[:-1] + apBl[:, 1] + aDl[:,state]
    return alphal
