...
namespace hlm
{
    ...
    template <typename Type>
    void internal_messages_forwards_log_prosody(
      int T, int L, int P, int W, int state, Type *apBl, Type* aDl,
      Type *alphal)
    {
      // T: Length of observations.
      // P: Number of states F_t
      // W: Number of words in model. (Number of upper limit of words.)
      // L: Length of the word. (Number of letters in word.)
      NPArray<Type> eapBl(apBl, T, P);
      NPArray<Type> eaDl(aDl, T, W);
      ...
      Type ctmp = 0.0;
      for(int t=0; t<T; t++){
        ealphal(t, L-1) = ctmp + eapBl(t, 1) + eaDl(t, state);
        ctmp = ctmp + eapBl(t, 0);
      }
    }
}
...
class hlmc
{
    ...
    static void internal_messages_forwards_log_prosody(
      int T, int L, int P, int W, int state, FloatType *apBl, FloatType *aDl,
      FloatType *alphal)
    { hlm::internal_messages_forwards_log_prosody(T, L, P, W, state, apBl, aDl, alphal); }
