...
class WeakLimitProsodicHDPHLMPython(WeakLimitHDPHLMPython):
    ...
    def __init__(self, hypparams, letter_hsmm, dur_distns, length_distn, prosody_distns):
        self._prosody_distns = prosody_distns
        return super(WeakLimitProsodicHDPHLMPython, self).__init__(hypparams, letter_hsmm, dur_distns, length_distn)

    @property
    def prosody_distns(self):
        return self._prosody_distns

    def add_data(self, data, prosody, **kwargs):
        self.states_list.append(self._states_class(self, data, prosody, **kwargs))
    ...
    def resample_prosody_distns(self):
        ...
        for s in self.states_list:
            ...
            F_prosody[0].append(s.prosody[F_t[0]])
            F_prosody[1].append(s.prosody[F_t[1]])
    ...
    def _joblib_resample_states(self,states_list,num_procs):
        ...
        if len(states_list) > 0:
            joblib_args = list_split(
                    [self._get_joblib_pair_prosody(s) for s in states_list],
                    num_procs)
            ...
            raw_stateseqs = Parallel(n_jobs=num_procs,backend='multiprocessing')\
                    (delayed(parallel._get_sampled_stateseq_norep_and_durations_censored_prosody)(idx)
                            for idx in range(len(joblib_args)))
    ...
    def _get_joblib_pair_prosody(self,states_obj):
        return (states_obj.data, states_obj.prosody, states_obj._kwargs)

class WeakLimitProsodicHDPHLM(WeakLimitProsodicHDPHLMPython):
    _states_class = hlm_states.WeakLimitProsodicHDPHLMStates
